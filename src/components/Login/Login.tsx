...
import { Input } from "../../common/Preloader/FormsControl";
...
const LoginForm: React.FC<InjectedFormProps<LoginFormDataType, LoginFormOwnProps> & LoginFormOwnProps>  = ({handleSubmit, error, captchaUrl}) => {
    return (
        <form onSubmit={handleSubmit} className={style.login__form}>
             ...
             <div className={style.from_field__container}><Field className={style.login__field_checkbox} type={"checkbox"} name={'rememberMe'} component={Input}  validate={[requiredField]} /><span> remember me</span></div>  
            {error && 
